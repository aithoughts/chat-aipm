---
title: AiPMChat Plugin Development Guide
description: >-
  Learn how to create and integrate custom plugins in AiPMChat, including plugin
...
1. **Plugin Index**: Used to display basic information about the plugin, including the plugin name, description, author, version, and a link to the plugin manifest. The official plugin index can be found at [lobe-chat-plugins](https://github.com/aipmhub/aipm-chat-plugins). If you want to publish a plugin to the official plugin marketplace, you need to [submit a PR](https://github.com/aipmhub/aipm-chat-plugins/pulls) to this repository.
...
This section will introduce how to add and use a custom plugin in AiPMChat.
...
### Add Local Plugin in AiPMChat Role Settings

Next, go to AiPMChat, create a new assistant, and go to its session settings page:

<Image
  alt="Add Local Plugin in AiPMChat Role Settings S1"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in AiPMChat Role Settings S2"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in AiPMChat Role Settings S3"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in AiPMChat Role Settings S4"
  height={483}
  inStep
...
After completing these operations, you have understood the basic process of adding custom plugins and using them in AiPMChat.
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address AiPMChat loads the plugin's front-end interface. Since AiPMChat plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for AiPMChat to query the plugin's API interface. AiPMChat's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, AiPMChat will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
For the Edge Runtime, we provide the `createErrorResponse` method in `@aipmorg/chat-plugin-sdk` to quickly return error responses. Currently, the provided error types are detailed in: [PluginErrorType][plugin-error-type-url].
...
Since the default plugin gateway for AiPMChat is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), AiPMChat> will directly request this address, and then only the corresponding gateway needs to be created at that address.
...
[`@aipmorg/chat-plugins-gateway`](https://github.com/aipmhub/chat-plugins-gateway) contains the implementation of the plugin gateway in AiPMChat [here](https://github.com/aipmhub/aipm-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing AiPMChat to access the local plugin service.
...
AiPMChat implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@aipmorg/chat-plugin-sdk`](https://github.com/aipmhub/chat-plugin-sdk) to simplify communication between the plugin and AiPMChat. You can actively retrieve the current message data from AiPMChat using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].

```tsx
import { fetchPluginMessage } from '@aipmorg/chat-plugin-sdk';
...
If you want more people to use your plugin, feel free to [submit it for listing](https://github.com/aipmhub/aipm-chat-plugins) on the plugin marketplace.

[![][submit-plugin-shield]][submit-plugin-url]

### Plugin Shield

[![lobe-chat-plugin](https://img.shields.io/badge/%F0%9F%A4%AF%20%26%20%F0%9F%A7%A9%20AiPMHub-Plugin-95f3d9?labelColor=black&style=flat-square)](https://github.com/aipmhub/aipm-chat-plugins)
...
- **📘 Pluging SDK Documentation**: [https://chat-plugin-sdk.theforage.cn](https://chat-plugin-sdk.theforage.cn)
- **🚀 chat-plugin-template**: [https://github.com/lobehub/chat-plugin-template](https://github.com/lobehub/chat-plugin-template)
- **🧩 chat-plugin-sdk**: [https://github.com/aipmhub/chat-plugin-sdk](https://github.com/aipmhub/chat-plugin-sdk)
- **🚪 chat-plugin-gateway**: [https://github.com/aipmhub/chat-plugins-gateway](https://github.com/aipmhub/chat-plugins-gateway)
- **🏪 lobe-chat-plugins**: [https://github.com/aipmhub/aipm-chat-plugins](https://github.com/aipmhub/aipm-chat-plugins)
...
[manifest-docs-url]: https://chat-plugin-sdk.theforage.cn/guides/plugin-manifest
[plugin-error-type-url]: https://github.com/lobehub/chat-plugin-template
[submit-plugin-shield]: https://img.shields.io/badge/🧩/🏪_submit_plugin-%E2%86%92-95f3d9?labelColor=black&style=for-the-badge
[submit-plugin-url]: https://github.com/aipmhub/aipm-chat-plugins
