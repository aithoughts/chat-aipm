---
title: AiPMChat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for AiPMChat Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest AiPMChat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running AiPMChat Container

Assuming the AiPMChat container is named `lobe-chat`, use the following commands to stop and remove the currently running AiPMChat container:
...
### Pull the Latest AiPMChat Image

Use the following command to pull the latest Docker image for AiPMChat:
...
Redeploy the AiPMChat container using the newly pulled image:
...
No need to worry, you won't. All of AiPMChat's chat records are stored in your local browser. Therefore, when redeploying AiPMChat using Docker, your chat records will not be lost.
