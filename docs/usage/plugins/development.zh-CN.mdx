...
[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) 包含了 AiPMChat 中插件网关的[实现](https://github.com/aipmhub/aipm-chat/blob/main/src/pages/api/plugins.api.ts)，你可以直接使用该包创建网关，进而让 AiPMChat 访问到本地的插件服务。
...
其中关于插件通信，我们在 [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) 提供了相关方法，用于简化插件与 AiPMChat 的通信。你可以通过 `fetchPluginMessage` 方法主动向 AiPMChat 获取当前消息的数据。关于该方法的详细介绍，参见：[fetchPluginMessage][fetch-plugin-message-url]。
